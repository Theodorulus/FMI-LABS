...
add2 :: [Arb] -> Stare -> Val
add2 (h:t) env = fromMaybe 0 (eval1 h env) + add2 t env
add2 [] env = 0


eval1 :: Arb -> Stare -> Maybe Val
eval1 (Leaf v) env = val1 v env
eval1 (Node Add s) env = Just (add2 s env)
eval1 (Node (If op1 op2) s) env = fct_if s op1 op2 env where
    fct_if (h:t) op1 op2 env = 
        if test0 (eval1 h env) then
            eval1 (Node op1 t) env
        else
            eval1 (Node op2 t) env
    fct_if [] op1 op2 env = Nothing




val1 :: Lit -> Stare -> Maybe Val
val1 (V v) _ = return v
val1 (Id nume) env = lookup nume env


test0 :: Maybe Val -> Bool
test0 (Just x) = x /= 0
test0 Nothing = False 
