...
float const U_MIN_CON = -PI, U_MAX_CON = PI, V_MIN_CON = PI * -120, V_MAX_CON = 20 * PI;
// (2) numarul de paralele/meridiane, de fapt numarul de valori ptr parametri
...
void RenderFunction(void)
{
	...
	view = glm::lookAt(Obs, PctRef, Vert) *glm::rotate(glm::mat4(1), PI * 0.7f, glm::vec3(1.f, 0.f, 0.0f));
	// view = view * glm::translate(glm::mat4(1.f), glm::vec3(0, 0, 74.0f));
	glUniformMatrix4fv(viewLocation, 1, GL_FALSE, &view[0][0]);
	...
	{
		glm::mat4 current_view = view * glm::rotate(glm::mat4(1), (2 * PI / nr_frunze) * frunza, glm::vec3(0.f, 1.f, 0.f)) * glm::translate(glm::mat4(1.f), glm::vec3(0.001f * frunza, 0, 74.0f));
		...
		for (int patr = 0; patr < (NR_PARR + 1) * NR_MERID; patr++)
		{
			if ((patr + 1) % (NR_PARR + 1) != 0) // nu sunt considerate fetele in care in stanga jos este Polul Nord
				//glDrawElementsInstanced(GL_QUADS, 4, GL_UNSIGNED_SHORT, (GLvoid*)((2 * (NR_PARR + 1) * (NR_MERID)+4 * patr) * sizeof(GLushort)), 20);
