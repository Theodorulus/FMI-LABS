...
void CreateVBO2(void)
{
	...
	// varfurile 
	// (4) Matricele pentru varfuri, culori, indici
	glm::vec4 Vertices[(NR_PARR_CON + 1) * NR_MERID_CON];
	glm::vec3 Colors[(NR_PARR_CON + 1) * NR_MERID_CON];
	GLushort Indices[2 * (NR_PARR_CON + 1) * NR_MERID_CON + 4 * (NR_PARR_CON + 1) * NR_MERID_CON];
...
void CreateVBOsnow(void)
{
	// Varfurile 
	GLfloat Vertices[] =
	{
		// background
		-300.0f,  -1500.0f, -750.0f, 1.0f,
		-300.0f,  -1500.0f,  -350.0f, 1.0f,
		-300.0f,  1500.0f, -350.0f, 1.0f,
		-300.0f,  1500.0f,  -750.0f, 1.0f,
	};
	...
	for (int n = 0; n < INSTANCE_COUNT; n++)
	{
		MatModel[n] = glm::translate(glm::mat4(1.0f), glm::vec3(300, 2000, 300));
	}
...
{
	glClearColor(0.0f, 0.05f, 0.2f, 0.0f); // culoarea de fond a ecranului
...
void RenderFunction(void)
{
	...
	for (int strat = 0; strat < nr_straturi; strat++) {
		...
		for (int frunza = 0; frunza < nr_frunze; frunza++)
		{
			...
			glm::mat4 current_view = view * scaling(glm::pow(1.1, (float)strat) -0.4)*  glm::rotate(glm::mat4(1), (2 * PI / nr_frunze) * frunza, glm::vec3(0.f, 0.f, 1.f)) * offset(0, -(50+(float)strat/10), 130-(float)strat*(180/ (float)nr_straturi)) * leaning;
	...
	Obs = glm::vec3(Obsx, Obsy, Obsz);   // se schimba pozitia observatorului
	PctRef = glm::vec3(Refx, Refy, Refz); // pozitia punctului de referinta
	Vert = glm::vec3(Vx, Vy, Vz); // verticala din planul de vizualizare 

	view = glm::lookAt(Obs, PctRef, Vert);
	glUniformMatrix4fv(viewLocation, 1, GL_FALSE, &view[0][0]);

	glDrawElements(GL_POLYGON, 4, GL_UNSIGNED_BYTE, 0);

	//glDrawElements(GL_POLYGON, 4, GL_UNSIGNED_BYTE, (GLvoid*)(4 * sizeof(GLubyte)));
